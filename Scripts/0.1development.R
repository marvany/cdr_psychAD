...
print(paste0('In total you have ', length(names(psychAD)), ' columns in df psychAD.'))
print(paste0('In total you have ', sum(names(psychAD) %in% names(df.merged)), ' overlapping columns between df psychAD and the merged table.'))
...
combo.grid = combo.grid[1:10]
# MAKE THIS ITERATE
#pbmcapply::pbmc
results = rbindlist(lapply(seq(nrow(combo.grid)), function(i){
    ...
    if (anl == "linear") {
          ...
          create_coeffiecients_df(fit = fit, fit.lm = fit.lm, phen = phen, cmp = cmp, anl = anl)
          ...
          create_coeffiecients_df(fit = NULL, fit.lm = fit.lm, phen = phen, cmp = cmp, anl = anl)
    }
}))
...
# Export results 
fwrite(results, 'results/assoc_anal_psychAD.csv')
